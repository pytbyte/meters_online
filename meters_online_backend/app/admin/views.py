...
def login():
    ...
    if admin_:
        ...
        return jsonify(jwtToken=access_token, customer_data=customer_data ,
                       access_token=access_token, user_type='admin', company_data=company_data), 200
...
@Admin.route('/<int:admin_id>/password', methods=['PUT'])
@jwt_required()
def update_Admins_password(admin_id):
    Admins_ = Admins.get_Admins_by_id(admin_id)
    if not Admins_:
        return jsonify(message='Admins not found'), 404
...
@Admin.route('/<int:admin_id>/role', methods=['PUT'])
@jwt_required()
def update_Admins_role(admin_id):
    Admins_ = Admins.get_Admins_by_id(admin_id)
    if not Admins_:
        return jsonify(message='Admins not found'), 404
    role = request.json.get('role', None)
